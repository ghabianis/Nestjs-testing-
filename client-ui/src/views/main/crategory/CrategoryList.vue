      ...
      >
        <el-table-column :prop="'id'" :label="'id'">
          <template #default="scope">
            <router-link
              :to="`/${entityPluralName}/${crategoryList[scope.$index].id}`"
              >{{ crategoryList[scope.$index].id }}</router-link
            >
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column prop="actions" align="right" width="150">
          <template #default="scope">
            ...
            <el-button
              @click="
                () => handleOpenConfirmModal(crategoryList[scope.$index]?.id)
              "
              icon="Delete"
              circle
              plain
            ...
            <ConfirmModal
              ...
              :isOpenModal="
                isOpenModal && modalId === crategoryList[scope.$index].id
              "
              @close-confirm-modal="
                isOpenModal = false;
                modalId = null;
              ...
              @approve-confirm-modal="
                () => handleRemoveEntity(crategoryList[scope.$index]?.id)
              "
            >
...
export default defineComponent({
  ...
  setup() {
    const entityPluralName = "crategories";
    const fieldsName = ref(["name", "type", "ref", "products"]);
    const isOpenModal = ref(false);
    const modalId = ref<string | null>(null);
    const { crategoryList, pagination, isLoading } = storeToRefs(
      useCrategoryStore()
    );
    const { fetchCrategories, deleteCrategory } = useCrategoryStore();

    const handleRemoveEntity = async (id: string) => {
      await deleteCrategory(id);
    };
